...
export function addToCart(product){
    ...
    const cart = getCart();
    const existingProductIndex = cart.findIndex((item) => { return item.productID === product.productID});
    ...
    if(existingProductIndex == -1){
       ...
       if(newQty <= 0 ) {
            const newCart = cart.filter((item,index) => {return index !== existingProductIndex});
